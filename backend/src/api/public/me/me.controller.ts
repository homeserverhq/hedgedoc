...
export class MeController {
  ...
  @OpenApi({
    code: 200,
    description:
      "Metadata of notes accessible to the user. Scope: my (owned), shared (shared with user), public (public notes), pinned (pinned by user). Default: my.",
    isArray: true,
  ...
  async getMyNotes(
    ...
    if (!Object.values(NoteScope).includes(scope as NoteScope)) {
      throw new BadRequestException(
        `Invalid scope: ${scope}. Must be one of: ${Object.values(NoteScope).join(", ")}`,
      );
    }
    const noteIds = await this.notesService.getNoteIdsByScope(
      scope as NoteScope,
      userId,
    );
